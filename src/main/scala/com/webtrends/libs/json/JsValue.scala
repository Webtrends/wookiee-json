...
case class JsResultException(errors: Seq[(JsPath, Seq[Any])]) extends RuntimeException("JsResultException(errors:%s)".format(errors))
...
private[json] class WookieeDeserializers(classLoader: ClassLoader) extends Deserializers.Base {
...
private[json] class WookieeSerializers extends Serializers.Base {
...
private[json] object JacksonJson {
  ...
  object module extends SimpleModule("WookieeJson", Version.unknownVersion()) {
    override def setupModule(context: SetupContext) {
      context.addDeserializers(new WookieeDeserializers(classLoader))
