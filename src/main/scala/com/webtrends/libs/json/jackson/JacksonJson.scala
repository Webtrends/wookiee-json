...
package com.webtrends.libs.json.jackson
...
import com.webtrends.libs.json._

import scala.annotation.{ switch, tailrec }
import scala.collection.mutable.ListBuffer

/**
  * The Wookiee JSON module for Jackson.
  *
  ...
  *
  *   val mapper = new ObjectMapper().registerModule(WookieJsonModule)
  *   val jsValue = mapper.readValue("""{"foo":"bar"}""", classOf[JsValue])
  * }}}
  */
object WookieeJsonModule extends SimpleModule("WookieeJson", Version.unknownVersion()) {
  override def setupModule(context: SetupContext) {
    context.addDeserializers(new WookieeDeserializers)
    context.addSerializers(new WookieeSerializers)
  }
}
...
private[jackson] class WookieeDeserializers extends Deserializers.Base {
...
private[jackson] class WookieeSerializers extends Serializers.Base {
...
private[json] object JacksonJson {
  ...
  private val mapper = (new ObjectMapper).registerModule(WookieeJsonModule)
